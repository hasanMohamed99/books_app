...
class Api {
  Future<dynamic> get ({required String url, String? token}) async {
    ...
    if (token != null) {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('there error with status code ${response.statusCode}');
      }
  }
  ...
  Future<dynamic> post ({required String url, @required dynamic body, String? token}) async {
    ...
    if(response.statusCode == 200){
      ...
      throw Exception('problem in status code ${response.statusCode} with body ${jsonDecode(response.body)}');
    }
  }
